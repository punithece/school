...
export class EmployeeService {
  ...
  public getEmployeeDtls(){
  
    //const queryStr = `${toDataSourceRequestString(state)}`;
    //let options = new RequestOptions({ headers: headers });
    //return this.http.get("http://localhost:8080/springboot-first-app-testng-1/api/getEmployeDtls", { headers }).pipe(map(responseType => Response ));
    return this.http.get(AppUrlFiles.APP_URL_AWS_LOCAL+"api/getEmployeDtls");
  }
